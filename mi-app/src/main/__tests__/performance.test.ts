...
describe('Métricas de Performance CRUD', () => {
  ...
  it('1. Tamaño del paquete final (MB)', async () => {
    ...
    try {
      ...
      console.log(`🏗️ Build (dist/out): ${((sizes['dist'] + sizes['out']) / 1024 / 1024).toFixed(2)} MB`)
  ...
  it('2. Consumo de RAM en idle y bajo carga', async () => {
    ...
    console.log(`✅ MÉTRICA: RAM idle = ${idleRSS.toFixed(2)} MB, RAM carga = ${loadRSS.toFixed(2)} MB`)
  ...
  it('3. Tiempo de arranque (simulado)', async () => {
    ...
    console.log(`⏱️ Tiempo de inicialización: ${initResults.ms.toFixed(2)}ms`)
    console.log(`🚀 Tiempo total simulado: ${totalStartupTime.toFixed(2)}ms`)
    console.log(`📝 Notas encontradas: ${initResults.data.notesCount}`)
    console.log(`🧠 Memoria post-arranque: ${(initResults.data.memoryUsed / 1024 / 1024).toFixed(2)} MB`)
  ...
  it('4. Tiempo de operaciones CRUD', async () => {
    ...
    console.log(`✅ Seed completado: ${seedTime.toFixed(2)}ms (${(seedTime/N).toFixed(2)}ms por nota)`)
    ...
    console.log(`👁️ Leyendo ${N} notas individuales...`)
    const readStart = performance.now()
    
    const readPromises = createdNotes.map(note => 
      notesStore.getNote(note.id)
    )
    await Promise.all(readPromises)
    const readTime = performance.now() - readStart
    
    console.log(`✅ Lectura completada: ${readTime.toFixed(2)}ms (${(readTime/N).toFixed(2)}ms por nota)`)
    
    // 4. ACTUALIZAR 10% de las notas
    const updateCount = Math.floor(N * updatePercent / 100)
    console.log(`✏️ Actualizando ${updateCount} notas (${updatePercent}%)...`)
    ...
    console.log(`✅ Actualización completada: ${updateTime.toFixed(2)}ms (${(updateTime/updateCount).toFixed(2)}ms por nota)`)
    
    // 5. ELIMINAR 10% de las notas
    const deleteCount = Math.floor(N * deletePercent / 100)
    console.log(`🗑️ Eliminando ${deleteCount} notas (${deletePercent}%)...`)
    const deleteStart = performance.now()
    
    const deletePromises = createdNotes.slice(updateCount, updateCount + deleteCount).map(note =>
      notesStore.deleteNoteById(note.id)
    )
    await Promise.all(deletePromises)
    const deleteTime = performance.now() - deleteStart
    
    console.log(`✅ Eliminación completada: ${deleteTime.toFixed(2)}ms (${(deleteTime/deleteCount).toFixed(2)}ms por nota)`)
  ...
  it('5. Tiempo de build/packaging', async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('⚠️ Build completo no disponible, midiendo proceso TypeScript...')
        ...
        try {
          ...
          console.log(`⏱️ Tiempo estimado: ${estimatedBuildTime.toFixed(2)}ms`)
